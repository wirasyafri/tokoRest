...
public function __construct()
	...
	public function index(){
		...
		$data['biodata_array']=$this->Pegawai_model->getgame();
	...
	public function Create(){
		...
		if($this->form_validation->run()==FALSE){
			$this->load->view('admin/tambah_game_view');
		}
		else{
			...
			if ( ! $this->upload->do_upload()){
				...
				$this->load->view('tambah_game_view',$error);
			}
			else{
		...
		$this->load->library('image_lib', $config); 
			...
			$this->load->view('admin/tambah_game_sukses');
			}
...
// 		//load model
		...
		if($this->form_validation->run()==FALSE){
			...
			if ( ! $this->upload->do_upload()){
				$error = array('error' => $this->upload->display_errors());
				$this->load->view('edit_game_view',$error);
			}
			else{
				$image_data = $this->upload->data();
						$configer = 
		...
		'height'   => 400,
			...
			$this->image_lib->initialize($configer);
			$this->image_lib->resize();
			$this->Pegawai_model->UpdateGame($id);
			$this->load->view('admin/edit_game_sukses');
		}
		}
	}
	public function delete($id){
		$this->Pegawai_model->delete($id);
		$this->load->view('admin/hapus_game_sukses');
	...
	$data['biodata_array']=$this->Pegawai_model->getBiodataQueryArray();
		...
		$this->load->view('admin/pegawai_datatable',$data);
	}
	...
	public function datatable_ajax(){
		$this->load->view('admin/pegawai_datatable_ajax');
	}
